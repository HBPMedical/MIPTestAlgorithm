---
title: "Histogram testing Notebook"
output:
...
### Histogram of right ententorhinal area

- Plot histogram

To define the bin size I use the Freedman–Diaconis rule (see: https://en.wikipedia.org/wiki/Freedman%E2%80%93Diaconis_rule)
It could be cool to have this value by default in the MIP.
...
size_H0 <- nclass.FD(df_H0$rightententorhinalarea) +1 # 36
...
### Histogram of right ententorhinal area by gender
...
size_H1 <- max(
  nclass.FD(unlist(df_H1_M)), nclass.FD(unlist(df_H1_W))) +1 # 25
...
### Histogram of right ententorhinal area by alzheimer broad category
...
result_H2$y_CN
result_H2$y_Other
sprintf("%0.7s - %0.7s", result_H2$xmax, result_H2$xmin)
```
...
### Bar graph of alzheimer broad category
...
### Bar graph of alzheimer broad category by gender
